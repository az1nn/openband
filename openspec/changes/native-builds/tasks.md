...
- [ ] **OPEN:** Run `cd android && ./gradlew assembleRelease` — documented command + config-coherent; NOT executed here (full Gradle toolchain / SDK unavailable)
- [ ] **OPEN:** Confirm `android/app/build/outputs/apk/release/app-release.apk` is produced
...
- [ ] **OPEN:** Run `cd electron && npm run build:linux` — documented command + config-coherent; NOT executed (`electron-builder` fetches the Electron binary)
- [ ] **OPEN:** Confirm `electron/out/` distributables exist
...
- [ ] **OPEN:** Device path — `Platform.OS !== "web"` branch uses `expo-audio` `AudioRecorder` writing into an armed `TrackDef` region; the native `hardwareIO` bridge path is in place so a future recorder can persist routes through `OpenBandNative`
...
- [ ] **OPEN:** `hardwareIO` returns real device list inside Electron (requires `node-audiodevice` binding; falls back to `[]`)
