...
|---|---|---|
| Guard AudioContext | `src/lib/universalAudio.ts` | `initialize()` at line ~127: guard `new AudioContext()` with `typeof AudioContext !== "undefined"` (in addition to the existing `Platform.OS === "web" && typeof window !== "undefined"` check) |
...
| CI version alignment | `package.json` | add `"engines": { "node": ">=22" }`; optional root `.nvmrc` with `22` |
...
- If exit is already 0 (`# tests 1421 | 1421 passed` on Node 22.23.x): the exit-1 from CI was environment/version-dependent (older commit, 1410 tests). Proceed with the hardening below anyway — it removes the root cause of the stray rejection and makes any future occurrence visible. Document the baseline in the PR.
...
The constructor currently runs whenever `Platform.OS === "web"` and `window` exists. In jsdom both are true but `AudioContext` is undefined. Change the availability check so `initialize()` **no-ops in environments without Web Audio**:
...
- Add `"engines": { "node": ">=22" }` to root `package.json` (documented minimum) and a root `.nvmrc` containing `22`, so local dev and CI share the same major.
...
- `npx vitest run; echo $?` exits **0** on Node 22 with `# tests 1421 | 1421 passed` and **no** "Unhandled Rejection" in the output (baseline recorded per §1).
