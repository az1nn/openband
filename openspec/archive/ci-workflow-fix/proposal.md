...
- **Make unhandled errors visible & non-fatal**: add an `onUnhandledError(err)` hook to `tests/ok-reporter.ts` that `console.error`s the error (so CI surface shows the actual rejection instead of a silent exit 1) and returns `false` to drop it from vitest's `errorsSet`; the vitest reporter API exposes no `onUnhandledRejection` hook (rejections route through `onUnhandledError`). **Authoritative exit gate:** `dangerouslyIgnoreUnhandledErrors: true` in `vitest.config.ts` (cli-api `_checkUnhandledErrors`), so real assertion failures still exit 1 via `hasFailed` while stray unhandled errors never set `process.exitCode = 1`.
