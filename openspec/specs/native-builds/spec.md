...
**Android prebuild (`android/`)** is a complete Expo prebuild Gradle project: `settings.gradle`, `app/build.gradle`, `app/src/main/java`, `app/src/main/AndroidManifest.xml`, and `debug.keystore` (`android/app/debug.keystore`) are present. Release output runs `cd android && ./gradlew assembleRelease`, landing at `android/app/build/outputs/apk/release/app-release.apk`. If absent, `android/local.properties` must point `sdk.dir` at the local Android SDK (documented in BUILD.md; not present in this env).
