...
export function RecordOptions({
  ...
  return (
    <View
      testID={testID}
      className="absolute inset-0 z-50 bg-black/60 justify-end"
    >
      <View className="bg-dark-elevated border-t border-dark-border rounded-t-3xl max-h-[80%]">
        <View className="flex-row items-center justify-between px-5 py-4 border-b border-dark-border">
          <View className="flex-row items-center gap-2">
            <View
              className={`w-3 h-3 rounded-full ${settings.armed ? "bg-red-500" : "bg-gray-600"}`}
            />
        ...
        <ScrollView className="px-5 py-4">
          ...
          >
            <Text
              className={`font-semibold ${settings.armed ? "text-red-400" : "text-gray-400"}`}
            >
              {settings.armed ? "Armado — Pronto para gravar" : "Desarmado"}
            </Text>
            <View
              className={`w-12 h-6 rounded-full p-0.5 ${settings.armed ? "bg-red-500" : "bg-dark-muted"}`}
            >
              <View
                className={`w-5 h-5 rounded-full bg-white shadow-sm ${settings.armed ? "ml-6" : "ml-0"}`}
              />
            </View>
          </Pressable>
          ...
          <View className="flex-row gap-2 mb-4">
            {(["low", "medium", "high", "lossless"] as const).map((q) => (
              ...
              >
                <Text
                  className={`text-sm ${settings.quality === q ? "text-brand-primary" : "text-gray-500"}`}
                >
                  {QUALITY_ICONS[q]}
                </Text>
                <Text
