...
import {
  ...
  buildMasteringChain as buildSuiteChain,
  createVersion,
  formatFileSize,
  formatSampleRate,
...
describe("projectStore.ts", () => {
  ...
  it("saveProject stores to localStorage", async () => {
    const { saveProject, loadProject } =
      await import("../src/lib/projectStore");
  ...
  it("listProjectIndex returns stored projects", async () => {
    const { saveProject, listProjectIndex } =
      await import("../src/lib/projectStore");
    saveProject("proj-1", mockProject);
  ...
  it("exportProject returns JSON string", async () => {
    const { saveProject, exportProject } =
      await import("../src/lib/projectStore");
    saveProject("proj-1", mockProject);
  ...
  it("importProject parses valid JSON and saves", async () => {
    const { importProject, loadProject } =
      await import("../src/lib/projectStore");
...
describe("generateTracksForGenre", () => {
  ...
  it("bpm parameter affects region duration", () => {
    ...
    for (let i = 0; i < fast.length; i++) {
      expect(fast[i].regions[0].duration).toBeLessThan(
        slow[i].regions[0].duration,
      );
    }
  });
  ...
  it("generates MIDI notes for melodic tracks with key parameter", () => {
    ...
    const melodicTracks = tracks.filter(
      (t) => t.midiNotes && t.midiNotes.length > 0,
    );
  ...
  it("bpm affects MIDI note start times and durations", () => {
    ...
    expect(slowTrack.midiNotes![1].start).toBeGreaterThan(
      fastTrack.midiNotes![1].start,
    );
    expect(slowTrack.midiNotes![0].duration).toBeGreaterThan(
      fastTrack.midiNotes![0].duration,
    );
  });
});
...
describe("mastering.ts", () => {
  ...
  it("Master Rápido has 3 plugins", () => {
    const master = MASTERING_CHAIN_PRESETS.find(
