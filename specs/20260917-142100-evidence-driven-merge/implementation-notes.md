...
4. **Design-drift protection** — the privileged evaluator binds `spec.md` and `plan.md` to the approved baseline and requires the effective risk-trigger set to exactly match the Design Gate contract; product-runtime, architecture-boundary and security-path changes add derived triggers and therefore fail closed when they were not approved.
5. **Recovery** — `docs/operations/merge-automation-recovery.md` defines disable, audit, revert and re-enable behavior.
